...
#'
#' @export
nougadmt.benchmark <- function(n=10240, k=30, d=40,
  snw = 3, spw = 1, nw = 1,
  ...
  t <- system.time(res <- nougadmt(received, spectra, snwf(spectra), spwf(spectra), nwf(spectra), alpha=alpha, accel=accel)$unmixed)
  ...
  plotf(main=paste0("nougadmt (elapsed: ",round(t['elapsed'], digits=3),")"), trans(res[,1:2]), pch='.', xlab='', ylab='')
}
