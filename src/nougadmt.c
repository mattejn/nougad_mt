...
void* cell_gd(void* td){
    ...
    size_t ni, di, ki, ii;
    float *lastg_k = malloc(sizeof(float) * k);//allocate vector of size k for grads
	for (ni = data->start; ni < data->stop; ++ni) {//for each cell (row)
    float *__restrict__ x_k = data->x_kn_c + ni * k; //declare x_k as the sole pointer to the abundandance vec(len=k) for curr cell
    float *__restrict__ r_d = data->r_dn_c + ni * d; //declare r_d as the sole pointer to the residual vec (len=d) for curr cell
    const float *__restrict__ y_d = data->y_dn_c + ni * d;//declare y_d as the sole pointer to the mixed vec(len=d) for curr cell
    ...
    for (ii = 0;; ++ii) { //for each iteration
      ...
      for (ki = 0; ki < k; ++ki)//for each spectrum
        for (di = 0; di < d; ++di) //for each detector in that spectrum
          r_d[di] += x_k[ki] * data->s_dk_c[di + d * ki];//increment the residual in that detector
          //by the curr spectrum val in that detector * curr spectrum abundance

      if (ii >= iters)
        break; //end if we reached maxiters
        //realistically we will never reach 0 residuals with unmixing on real data so that check is redundant

      for (ki = 0; ki < k; ++ki) {//for each spectra calculate gradient
        float gki = (x_k[ki] > 0 ? 0 : data->nw_k_c[ki] * x_k[ki]);//if abundance for curr spec is neg add ab*nw(for that spec) to grad
        for (di = 0; di < d; ++di)//for each detector in that spectrum
          gki +=//if residual in that detector is pos add res*spw to grad, otherwise res*snw
            r_d[di] * (r_d[di] > 0 ? data->spw_dk_c[di + d * ki] : data->snw_dk_c[di + d * ki]);
        ...
        x_k[ki] -= gki; //step against the gradient (decrease curr spec abundance by grad)
         ...
         float *r_dn)//starting residuals (mtx of 0s)
{
  ...
  bounds[threads-1].stop=n; //make sure we don't overshoot actual data length
  for (i=0; i<threads; i++) {//spawn threads and run their respective chunks
...
static const R_CMethodDef cMethods[] = { { "nougadmt_c", (DL_FUNC)&nougadmt_c, 14 },
                                         { NULL, NULL, 0 } };
